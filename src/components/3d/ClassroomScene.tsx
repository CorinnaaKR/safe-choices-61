...
export function ClassroomScene({ evidence, collectedIds, onCollectEvidence }: ClassroomSceneProps) {
  ...
  return (
    <group>
      ...
      {/* Jamie's desk highlighted - back right */}
      <group>
        <Desk position={[3.5, 0, 2.5]} />
        <Chair position={[3.5, 0, 2.95]} />
        {/* Subtle highlight under Jamie's desk */}
      ...
      {evidence.map((ev, i) => (
        <InteractiveObject
          key={ev.id}
          position={evidencePositions[i] || [i * 1.5, 0.9, 0]}
